...
function pagesinwidgets(){
	register_widget('page_section');
}

class page_section extends WP_Widget{
	function page_section(){
    	$widget_ops = array('classname' => 'page_section', 'description' => 'A general layout for page sections.' );
    	$this->WP_Widget('page_section', 'Page Widget', $widget_ops);
	}
	function form($instance){
		...
		<label for="<?php echo $this->get_field_id('titleEnable'); ?>-yes">Yes: <input type="radio" value="true" name="<?php echo $this->get_field_name('titleEnable'); ?>" id="<?php echo $this->get_field_id('titleEnable'); ?>-yes" <?php if(attribute_escape($instance['titleEnable']) == 'true'){echo 'checked="checked"';} ?>/></label>
		<label for="<?php echo $this->get_field_id('titleEnable'); ?>-no">No: <input type="radio" value="false" name="<?php echo $this->get_field_name('titleEnable'); ?>" id="<?php echo $this->get_field_id('titleEnable'); ?>-no" <?php if(attribute_escape($instance['titleEnable']) == 'false'){echo 'checked="checked"';} ?>/></label></p>
	...
	function widget($args, $instance){
		...
		$page = $page[0];
		if(attribute_escape($instance['titleEnable']) == 'true'){
